...
#region Curve
...
#if HACK
	private List<Vector2> CacheTerrainCurve = new List<Vector2>();
	
	public void AddPointMulti(int beforeWhichIndex)
	{
		List<e2dCurveNode> nodes = new List<e2dCurveNode>();
		int index = beforeWhichIndex > 0 ? beforeWhichIndex - 1 : beforeWhichIndex;
		Vector2 delta = CacheTerrainCurve[CacheTerrainCurve.Count - 1] - CacheTerrainCurve[0];
		for(int i=1; i < CacheTerrainCurve.Count; i++){
			Vector2 pos = CacheTerrainCurve[i] - CacheTerrainCurve[0] + TerrainCurve[index].position;
			e2dCurveNode node = new e2dCurveNode(pos);
			if (index > 0) node.texture = TerrainCurve[index - 1].texture;
			else if (index < TerrainCurve.Count) node.texture = TerrainCurve[index].texture;

			nodes.Add(node);
		}
		for (int i=beforeWhichIndex; i<TerrainCurve.Count; i++) {
			TerrainCurve[i].position += delta;
		}
		TerrainCurve.InsertRange(beforeWhichIndex, nodes);
		mCurveMesh.UpdateControlTextures();
	}
	
	public void RemovePointMulti(int l, int r)
	{
		CacheTerrainCurve.Clear();
		TerrainCurve.GetRange(l, r-l+1).ForEach(item => CacheTerrainCurve.Add(item.position));
		TerrainCurve.RemoveRange(l, r-l+1);
		mCurveMesh.UpdateControlTextures();
	}
#endif
	...
	{
		e2dCurveNode tmp = new e2dCurveNode(new Vector2(0,0));
		tmp.Copy(TerrainCurve[i]);
		TerrainCurve[i].Copy(TerrainCurve[j]);
		TerrainCurve[j].Copy(tmp);
	}
	
	void SortNodes(int l,int r)
	{
		
		if (l < r)
		{
			e2dCurveNode mid = new e2dCurveNode(new Vector2(0,0));
			mid.Copy(TerrainCurve[(l + r) / 2]);
			int i = l - 1;
			int j = r + 1;
			while (true)
			{
				while (TerrainCurve[++i].position.x < mid.position.x);
				while (TerrainCurve[--j].position.x > mid.position.x);
				if (i >= j) break;
				Swap(i, j);
			}
			SortNodes(l, i - 1);
	...
	{
		Debug.Log("Sort");
		SortNodes(0, TerrainCurve.Count - 1);
	}
#endif
