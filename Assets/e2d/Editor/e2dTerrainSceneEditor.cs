...
#if HACK
		if (!ShiftPressed() || !Event.current.alt)
		{
			ResetMulPoint();
		}
#endif
...
#region Curve
	...
	private void DrawNodesCursor()
	{
		...
		else // add new
		{
			...
			int pointIndex = GetCurvePointIndexWhereToAddNew(ControlPressed() && !Event.current.alt);
			if (pointIndex > 0)
			{
	...
	private void DrawCurveHandles()
	{
		...
		if (Event.current.alt)
		{
			...
			{
				if (Terrain.CurveClosed && i == Terrain.TerrainCurve.Count - 1) continue;
				Terrain.TerrainCurve[i].position += delta;
				Vector2 tmp = e2dEditorUtils.PositionHandle2dScaled(Terrain.TerrainCurve[i].position, handleScale, false);
				delta += tmp - Terrain.TerrainCurve[i].position;
				Terrain.TerrainCurve[i].position = tmp;
			}
		}
		else
		{
	...
	private void AddMulPoint()
	{
		Terrain.AddPointMulti(GetCurvePointIndexWhereToAddNew(false));
		Terrain.FixCurve();
		Terrain.FixBoundary();
		Terrain.RebuildAllMeshes();
	}
	
	int lMulPointToDel = -1,rMulPointToDel = -1;
	private void ResetMulPoint()
	{
		lMulPointToDel = rMulPointToDel = -1;
	}
	
	private void DeleteMulPoint()
	{
		if (lMulPointToDel == -1) {
			lMulPointToDel = GetCurvePointIndexToDelete();
		} else {
			rMulPointToDel = GetCurvePointIndexToDelete();
			if (lMulPointToDel > rMulPointToDel) {
				int tmp = lMulPointToDel;
				lMulPointToDel = rMulPointToDel;
				rMulPointToDel = tmp;
			}
			Terrain.RemovePointMulti(lMulPointToDel, rMulPointToDel);
			Terrain.FixCurve();
			Terrain.RebuildAllMeshes();
			ResetMulPoint();
		}
